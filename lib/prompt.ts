...
- Presenta las 3 opciones de menor a mayor precio. Para cada una, usa los campos del resultado:
  ...
  • Link al plan oficial: si el campo "pdf_url" viene con una URL, inclúyelo **tal cual, como URL completa y plana, sin acortar y sin usar markdown link**. Formato exacto: `📄 https://nuevamas.netlify.app/pdfs/.../codigo.pdf`. NUNCA escribas `[Ver PDF](url)` ni `[link](url)` — eso desaparece cuando el cliente copia el mensaje. Siempre la URL desnuda en cada una de las 3 opciones. Si viene **null o vacío** (no tenemos el PDF online por ahora), NO inventes una URL: omite la línea del PDF y al final di "📎 El PDF detallado del plan te lo envía el ejecutivo cuando se contacte contigo".
